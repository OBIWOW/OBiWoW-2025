...
We will load in the data as in Module 1. You probably still have the correct working directory and can proceed directly to `load()` the `.Rdata` file. 

If that doesn't work, in the Files tab on the bottom right panel, navigate to the folder `from-tidy-data-to-tidy-models-modeling-sensitive-health-data-in-r`. Click on the arrow beside the blue wheel and select "Set As Working Directory" from the contextual menu. This will execute a `setwd()` command in the Console to set the directory to the correct space to find the .RData file.
...
out_path<-paste0(getwd(),"/data/NHANES.csv")
write_csv(sub2,out_path)
#this file wrote into our working directory, within the data directory, and is named NHANES.csv
