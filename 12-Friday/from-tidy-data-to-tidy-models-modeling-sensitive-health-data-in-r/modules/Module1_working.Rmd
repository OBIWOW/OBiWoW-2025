...
## Part 2: Using readr to read in data
...
We are going to use example data from Software Carpentry. I have already downloaded it and included it in the `data/` directory within the repository. When you ran Setup.R, an R data space (`workshop_data.RData`) was created where I have already read the data frame into a data frame object called `dat`.

In the Files tab on the bottom right panel, navigate to the folder `from-tidy-data-to-tidy-models-modeling-sensitive-health-data-in-r`. Click on the arrow beside the blue wheel and select "Set As Working Directory" from the contextual menu. This will execute a `setwd()` command in the Console to set the directory to the correct space to find the .RData file.
...
#Use the blue wheel in the Files pane to "Set Working Directory as the root directory 
#this will execute a command in the Console to set the directory
#copy and paste the setwd command below to set the working directory within the .Rmd chunk if you want to execute code within the .Rmd
...
#let's look at the first few rows of the data frame called 'dat'


#we can accomplish the same thing with slightly different syntax
