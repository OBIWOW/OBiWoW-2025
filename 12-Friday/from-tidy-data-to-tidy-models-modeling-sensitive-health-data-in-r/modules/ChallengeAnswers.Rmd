...
```{r eval="false"}
######################################################################
###### FOR YOUR REFERENCE: READ IN DATA FROM DATA/ DIRECTORY ########
######################################################################
...
#this will execute a command in the Console that is like this
#setwd("/your/path/OBiWoW-2025/12-Friday/from-tidy-data-to-tidy-models-modeling-sensitive-health-data-in-r")
#place that command to set the working directory within the chunk if you want to execute code within the .Rmd
setwd("~/Documents/MyStuff/Misc/Coding/GitHub/OBiWoW-2025/12-Friday/from-tidy-data-to-tidy-models-modeling-sensitive-health-data-in-r")
...
idf<-idf %>% mutate(ID=row.names(idf))
idf %>% select(ID)
